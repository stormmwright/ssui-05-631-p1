...
class Target extends ScreenObject {
    ...
    pickedBy(ptX, ptY) {
        // === YOUR CODE HERE ===
        // check if point is within target using distance formula
        if (!this.visible)
            return false;
...
class Reticle extends Target {
    ...
    pickedBy(ptX, ptY) {
        // === YOUR CODE HERE ===
        // check if point is within inner reticle circle
        if (!this.visible)
            return false;
        const dx = ptX - this.centerX;
        const dy = ptY - this.centerY;
        return (dx * dx + dy * dy) <= ((Reticle.RETICLE_INNER_DIAM / 2) ** 2);
    }
    // . . . . . . . . . . . .  . . . . . . . . . . . . . . . . . . . . . .
...
class BackgroundDisplay extends ScreenObject {
    ...
    draw(ctx) {
        ...
        let xpos = 10;
        // === YOUR CODE HERE ===
        // dynamic approach where ypos of each subsequent message depends on font height and leading
        if (this.msg1) {
